...
class DeleteUserDto {
  ...
  @override
  String toString() => 'DeleteUserDto[callbackURL=$callbackURL, password=$password, token=$token]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DeleteUserDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DeleteUserDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
