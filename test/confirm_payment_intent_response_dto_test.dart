//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:flutter_cashful/api.dart';
import 'package:test/test.dart';

// tests for ConfirmPaymentIntentResponseDto
void main() {
  // final instance = ConfirmPaymentIntentResponseDto();

  group('test ConfirmPaymentIntentResponseDto', () {
    // PaymentIntentResponseDto paymentIntent
    test('to test the property `paymentIntent`', () async {
      // TODO
    });

    // Object iveriParams
    test('to test the property `iveriParams`', () async {
      // TODO
    });

    // String iVeri3dsEndpoint
    test('to test the property `iVeri3dsEndpoint`', () async {
      // TODO
    });

    // bool sandboxMode
    test('to test the property `sandboxMode`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // String transactionId
    test('to test the property `transactionId`', () async {
      // TODO
    });

    // String authCode
    test('to test the property `authCode`', () async {
      // TODO
    });

    // num amount
    test('to test the property `amount`', () async {
      // TODO
    });

    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // String reason
    test('to test the property `reason`', () async {
      // TODO
    });


  });

}
